  ...
  const renderDashboard = () => (
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 items-start">
        <Card className="bg-blue-600 text-white self-start"><CardContent className="flex items-center justify-between"><div className="flex items-center justify-between w-full"><div><p className="text-blue-100">Empleados Activos</p><p className="text-3xl font-bold">{usuarios.filter((e) => e.activo).length}</p></div><Users className="w-12 h-12 text-blue-200" /></div></CardContent></Card>
        <Card className="bg-green-600 text-white"><CardContent className="p-6 flex flex-col justify-center"><div className="flex items-center justify-between"><div><p className="text-green-100">Obras Activas</p><p className="text-3xl font-bold">{obras.filter((o) => o.estado === "en_progreso").length}</p></div><Building className="w-12 h-12 text-green-200" /></div></CardContent></Card>
        <Card className="bg-purple-600 text-white"><CardContent className="p-6 flex flex-col justify-center"><div className="flex items-center justify-between"><div><p className="text-purple-100">Clientes Totales</p><p className="text-3xl font-bold">{clientes.length}</p></div><Users className="w-12 h-12 text-purple-200" /></div></CardContent></Card>
        <Card className="bg-orange-600 text-white"><CardContent className="p-6 flex flex-col justify-center"><div className="flex items-center justify-between"><div><p className="text-orange-100">Ingresos Mes</p><p className="text-3xl font-bold">{formatCurrency(mockReportesVentas[mockReportesVentas.length - 1]?.ingresosBrutos || 0)}</p></div><DollarSign className="w-12 h-12 text-orange-200" /></div></CardContent></Card>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="cursor-pointer hover:shadow-lg transition-shadow" onClick={() => setCurrentSection("empleados")}><CardContent className="p-6 text-center flex flex-col items-center justify-center"><UserPlus className="w-12 h-12 text-blue-600 mx-auto mb-4" /><h3 className="font-semibold text-lg mb-2">Gestionar Empleados</h3><p className="text-gray-600">Crear, editar y administrar empleados</p></CardContent></Card>
        <Card className="cursor-pointer hover:shadow-lg transition-shadow" onClick={() => setCurrentSection("reportes")}><CardContent className="p-6 text-center flex flex-col items-center justify-center"><TrendingUp className="w-12 h-12 text-green-600 mx-auto mb-4" /><h3 className="font-semibold text-lg mb-2">Reportes de Ventas</h3><p className="text-gray-600">Análisis detallado de ventas</p></CardContent></Card>
        <Card className="cursor-pointer hover:shadow-lg transition-shadow" onClick={() => setCurrentSection("obras")}><CardContent className="p-6 text-center flex flex-col items-center justify-center"><Building className="w-12 h-12 text-purple-600 mx-auto mb-4" /><h3 className="font-semibold text-lg mb-2">Supervisar Obras</h3><p className="text-gray-600">Vista general de todas las obras</p></CardContent></Card>
      </div>
    </div>
  )
      ...
      <CardContent className="p-4 flex flex-col justify-between">
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex space-x-1"><Button onClick={() => handleViewEmpleado(empleado)} variant="ghost" size="icon" title="Ver detalles"><Eye className="w-4 h-4" /></Button><Button onClick={() => handleEditEmpleado(empleado)} variant="ghost" size="icon" title="Editar"><Edit className="w-4 h-4" /></Button><Button onClick={() => handleDeleteEmpleado(empleado.id)} variant="destructive" size="icon" title="Eliminar"><Trash2 className="w-4 h-4" /></Button></div>
          </div>
          <h4 className="font-semibold text-sm mb-1">{empleado.nombre} {empleado.apellido}</h4>
          <p className="text-xs text-gray-500 mb-2">{empleado.email}</p>
        </div>
        <div className="flex justify-between items-center">
          <span className={`px-2 py-1 rounded text-xs ${empleado.activo ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}>{empleado.activo ? "Activo" : "Inactivo"}</span>
        </div>
      </CardContent>
    </Card>
  )
