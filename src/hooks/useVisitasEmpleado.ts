...
export function useVisitasEmpleado(cuil: string | null | undefined) {
  ...
  const loadVisitas = useCallback(async () => {
    ...
    const results = await Promise.allSettled([
      getVisitasByEmpleado(cuil, ['PROGRAMADA']),
      getVisitasByEmpleado(cuil, ['COMPLETADA']),
    ])
    ...
    if (completadasResult.status === 'fulfilled') {
      ...
      console.error(
        'Error al cargar visitas completadas:',
        completadasResult.reason
      )
      // Si ya hay un error, podrías concatenarlo o decidir cuál es más importante
      setError((prevError) =>
        prevError
          ? `${prevError} Y tampoco las completadas.`
          : 'No se pudieron cargar las visitas completadas.'
      )
